...
% 1) Finds the optimal policy [mstar,mbar]
...
%% 4) Check that the optimal policy we found is a true global minimum
...
for j=1:n
    ...
    policy_opt_vec(j,i)={[policy_opt(1) policy_opt(2)]};
    fval_vec(j,i)={fval};
    end
end
...
if abs(mstar_opt_final-mstar_opt)<tol && abs(mbar_opt_final-mbar_opt)<tol
    disp('You found the optimal policy!')
else
    disp('You found a local minimum: change the initial condition at part (1).')
end
%% Saving figures
for i=1:1:9
